...
from analysis.spectra import compute_1d_spectrum, compute_cospectrum, compute_ogive
from tools.common import get_variable_time_axis
from tools.plotting import plot_cospectra, plot_energy_spectra, plot_ogives, plot_spectral_panel
...
COSPECTRUM_PAIRS = (("u", "w"), ("w", "tc"))
OGIVE_PAIRS = (("u", "w"), ("w", "tc"))
...
def _cospectra(data, config, pairs):
    _, time_local, _, _ = get_variable_time_axis(data)
    stats = []
    for prefix_x, prefix_y in pairs:
...
def _ogives(data, config):
    _, time_local, _, _ = get_variable_time_axis(data)
    stats = []
    for prefix_x, prefix_y in OGIVE_PAIRS:
        x_series = collect_height_series(data, config.site, prefix_x)
        y_series = collect_height_series(data, config.site, prefix_y)
        x_series, y_series = align_height_series(x_series, y_series)
        output = []
        for x_entry, y_entry in zip(x_series, y_series):
            frequency, values, _ = compute_cospectrum(
                x_entry["data"], x_entry["meta"], y_entry["data"], y_entry["meta"], time_local,
                apply_log_binning=APPLY_LOG_BINNING, log_bins_per_decade=LOG_BINS_PER_DECADE,
            )
            output.append({
                **x_entry,
                "timeScaleSeconds": 1.0 / frequency,
                "data": compute_ogive(frequency, values),
            })
        stats.append({"varName": f"{prefix_x}_{prefix_y}", "displayName": f"Og({prefix_x}, {prefix_y})", "series": output})
    return stats


def run(config_path: str | Path | None = None, flag_overrides: dict[str, bool] | None = None) -> list[PlotArtifact]:
    flags = resolve_flags(
        {"plot_spectra": PLOT_SPECTRA, "plot_cospectra": PLOT_COSPECTRA, "plot_ogive": PLOT_OGIVE,
    ...
    cospectra = _cospectra(data, config, COSPECTRUM_PAIRS) if flags["plot_cospectra"] or flags["plot_spectral_panels"] else []
    ogives = _ogives(data, config) if flags["plot_ogive"] else []
    artifacts = []
    ...
    if flags["plot_ogive"]:
        artifacts.append(artifact_from_figure(config, "ogive", plot_ogives(ogives, f"{config.site.upper()}: ogives"), flags["save_figures"]))
